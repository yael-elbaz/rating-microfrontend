...
- `isFirstHostRequest` is `true` on the first host request of **every application load** (F5, new tab, full navigation) and `false` for the rest of that load — it is an in-memory flag in `httpDecService.ts`, not a cookie. `isFirstMfeRequest` is `true` on each MFE's first request per **open**: the registry in `mfeContext.ts` tracks which MFEs have called, and the MFE clears its entry on unmount via `http.release()` (or `releaseMfe(idntObjectPPR)`), so closing and reopening the MFE reports `true` again. An MFE that never calls `release()` keeps `false` until the next page load. The only cookie left is `__GUID_O` (`sessionGuid`), which does persist for the whole browser session.
